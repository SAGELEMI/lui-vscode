...
// declaration snapshots on one GL context. Never loads game Lua or player storage.
...
const {buildDeclarationSnapshot,canonicalTag}=require('../dist/previewSnapshot.cjs');
...
local ok,err=xpcall(function()
 ...
 local runtime=setmetatable({config_={componentDirectories={}},documents_={},code_={},isV2_=true},Runtime)
 ...
 local content
 if '${kind}'=='source' then
  runtime.documents_['Fixture.lui']=assert(require('LUI.Parser').Parse(${long(source)},'Fixture.lui'))
  content=assert(runtime:RenderMarkup('Fixture.lui',context))
 ...
 cancel=runtime:AfterLayout(candidate,function()
  ...
  local out=VariantMap();out['name']='lui-parity-ready';out['payload']=cjson.encode({sequence='${seq}',minimumCompletedLayouts=frames,visibleLayout=settled,text=title and title.props.text,textRect=title and runtime:GetScreenRect(title),uiScale=UI.GetScale()})
...
if not ok then local out=VariantMap();out['name']='lui-parity-ready';out['payload']=cjson.encode({sequence='${seq}',error=tostring(err)});SendEvent('EmitToPlugin',out) end`);
 ...
 return {...capture,visibleLayout:result.visibleLayout,sequence:seq,minimumCompletedLayouts:result.minimumCompletedLayouts,textRect:rect,textRegionColors:colors.size,textInkPixels:inkPixels,stability:{firstRaf:first.completedVendorRaf,secondRaf:capture.completedVendorRaf,firstTimestamp:first.rafTimestamp,secondTimestamp:capture.rafTimestamp,distinctDisplayTimestamps:true,sha256:first.sha256}};
}
try{
 ...
 for(const [name,delta]of samples){
  ...
  const node={width:390,height:867,...buildDeclarationSnapshot(serialize(parsed.root),{view},{tag:n=>canonicalTag(n.tag)??n.tag,component:()=>undefined})};
