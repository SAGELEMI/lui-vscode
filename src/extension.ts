...
const vscode=acquireVsCodeApi();let model;let selected;let hovered;
...
/* Preview V2: semantic widgets only. Display names stay in the inspector, never on the canvas. */
function pick(start){selected=start;draw();}
function hover(start){hovered=start;applyHighlights();}
function applyHighlights(){document.querySelectorAll('[data-start]').forEach(el=>{const start=Number(el.dataset.start);el.classList.toggle('is-selected',start===selected);el.classList.toggle('is-hovered',start===hovered);});}
function bindingMatch(value){return /^\\{Binding\\s+([\\w.-]+)\\}$/.exec(value||'');}
function previewValue(path){const state=previews(model?.root).find(p=>p.start===Number(byId('preview').value));const set=(state?.children||[]).find(child=>child.tag==='lui:Set'&&child.attrs?.Path===path);return set?.attrs?.Value;}
function bindingFallback(path){const leaf=path.split('.').pop().toLowerCase();const labels={title:'页面标题',name:'名称',subtitle:'副标题',description:'说明',floor:'第 1 层',level:'1 级',coins:'0',health:'100',hp:'100',settings:'设置',back:'返回'};return labels[leaf]||'示例内容';}
function stateValue(value){const hit=bindingMatch(value);if(!hit)return value;const preview=previewValue(hit[1]);return preview===undefined?bindingFallback(hit[1]):preview;}
function conditionValue(value){const resolved=stateValue(value);return !['false','0','nil',''].includes(String(resolved).trim().toLowerCase());}
function effectiveAttrs(node){const attrs={...(node.attrs||{})};for(const child of node.children||[]){const hit=new RegExp('^'+String(node.tag).split('.').join('\\.')+'\\.([\\w-]+)$').exec(child.tag||'');if(hit)attrs[hit[1]]=(child.children||[]).filter(c=>c.kind==='text').map(c=>c.text).join('').trim();}return attrs;}
function visualChildren(node){return(node.children||[]).filter(child=>!child.tag?.startsWith(String(node.tag)+'.')&&child.tag!=='lui:Preview'&&child.tag!=='lui:Set');}
function cssSize(value){const text=String(value??'').trim();return /^-?\\d+(?:\\.\\d+)?$/.test(text)?text+'px':text;}
function cssBox(value){return String(value??'').trim().split(/\\s+/).filter(Boolean).map(cssSize).join(' ');}
function applyLayout(el,attrs){if(attrs.Width)el.style.width=cssSize(attrs.Width);if(attrs.Height)el.style.height=cssSize(attrs.Height);if(attrs.MinWidth)el.style.minWidth=cssSize(attrs.MinWidth);if(attrs.MinHeight)el.style.minHeight=cssSize(attrs.MinHeight);if(attrs.Margin)el.style.margin=cssBox(attrs.Margin);if(attrs.Padding)el.style.padding=cssBox(attrs.Padding);if(attrs.Gap)el.style.gap=cssSize(attrs.Gap);if(attrs.FlexGrow)el.style.flexGrow=String(attrs.FlexGrow);if(attrs.Background)el.style.background=attrs.Background;if(attrs.Opacity)el.style.opacity=String(attrs.Opacity);if(attrs.Align){const align={start:'flex-start',center:'center',end:'flex-end',stretch:'stretch'};el.style.alignItems=align[String(attrs.Align).toLowerCase()]||attrs.Align;}if(attrs.Justify){const justify={start:'flex-start',center:'center',end:'flex-end',between:'space-between',around:'space-around',evenly:'space-evenly'};el.style.justifyContent=justify[String(attrs.Justify).toLowerCase()]||attrs.Justify;}const positioned=attrs.Position==='absolute'||attrs.Left||attrs.Top||attrs.Right||attrs.Bottom||attrs.Anchor;if(!positioned)return;el.style.position='absolute';if(attrs.Left)el.style.left=cssSize(attrs.Left);if(attrs.Top)el.style.top=cssSize(attrs.Top);if(attrs.Right)el.style.right=cssSize(attrs.Right);if(attrs.Bottom)el.style.bottom=cssSize(attrs.Bottom);const anchors={'top-left':['0','0','translate(0,0)'],top:['50%','0','translateX(-50%)'],'top-right':['100%','0','translateX(-100%)'],left:['0','50%','translateY(-50%)'],center:['50%','50%','translate(-50%,-50%)'],right:['100%','50%','translate(-100%,-50%)'],'bottom-left':['0','100%','translateY(-100%)'],bottom:['50%','100%','translate(-50%,-100%)'],'bottom-right':['100%','100%','translate(-100%,-100%)']};const anchor=anchors[String(attrs.Anchor||'').toLowerCase()];if(anchor){if(!attrs.Left&&!attrs.Right)el.style.left=anchor[0];if(!attrs.Top&&!attrs.Bottom)el.style.top=anchor[1];el.style.transform=anchor[2];}}
function wireCanvasNode(el,node){el.dataset.start=node.start;el.title=node.displayName||node.tag||'';el.onclick=e=>{e.stopPropagation();pick(node.start);};el.onmouseenter=()=>hover(node.start);el.onmouseleave=()=>hover(undefined);}
function structuralNode(node,attrs,kind){const el=document.createElement('div');el.className='lui-node '+kind;applyLayout(el,attrs);wireCanvasNode(el,node);for(const child of visualChildren(node))el.append(renderNode(child));return el;}
function renderNode(node){if(node.kind==='text'){const span=document.createElement('span');span.textContent=node.text;return span;}if(node.kind==='comment')return document.createComment(node.text||'');const tag=node.tag||'Panel';if(tag==='lui:Preview'||tag==='lui:Set')return document.createDocumentFragment();const attrs=effectiveAttrs(node);if(tag==='lui:If'&&!conditionValue(attrs.Test))return document.createDocumentFragment();if(tag==='lui:If'||tag==='lui:For'||tag==='lui:Slot')return structuralNode(node,attrs,'condition');const kind=tag.replace('lui:','').toLowerCase();const el=document.createElement(tag==='Button'?'button':'div');el.className='lui-node '+kind;if(tag==='Button'&&String(attrs.Variant).toLowerCase()==='secondary')el.classList.add('secondary');applyLayout(el,attrs);wireCanvasNode(el,node);if(tag==='Text'||tag==='Button'){el.textContent=stateValue(attrs.Text||'');if(attrs.FontSize)el.style.fontSize=cssSize(attrs.FontSize);if(attrs.Color)el.style.color=attrs.Color;}else if(tag==='Progress'){const value=Number(stateValue(attrs.Value||0))||0,max=Math.max(1,Number(stateValue(attrs.Max||1))||1),track=document.createElement('div'),fill=document.createElement('i'),label=document.createElement('small');track.className='progress-track';fill.className='progress-fill';fill.style.width=Math.max(0,Math.min(100,value/max*100))+'%';label.textContent=Math.round(value)+' / '+Math.round(max);track.append(fill);el.append(track,label);}else if(tag==='Toggle'){const toggle=document.createElement('span');toggle.className='toggle '+(String(stateValue(attrs.Value||'')).toLowerCase()==='true'?'on':'');toggle.textContent=toggle.classList.contains('on')?'开启':'关闭';el.append(toggle);}else if(tag==='Slider'){const input=document.createElement('input');input.type='range';input.disabled=true;input.min=String(attrs.Min||0);input.max=String(attrs.Max||100);input.value=String(Number(stateValue(attrs.Value||0))||0);el.append(input);}for(const child of visualChildren(node))el.append(renderNode(child));return el;}
function outline(node,host){if(!node||node.kind!=='element'||node.tag==='lui:Preview'||node.tag==='lui:Set')return;const row=document.createElement('button');row.className='outline-row';row.dataset.start=node.start;row.textContent=node.displayName||node.tag;row.onclick=()=>pick(node.start);row.onmouseenter=()=>hover(node.start);row.onmouseleave=()=>hover(undefined);host.append(row);const kids=document.createElement('div');kids.className='outline-children';for(const child of visualChildren(node))outline(child,kids);if(kids.childElementCount)host.append(kids);}
function propertyInput(host,node,name,value,placeholder,options){const label=document.createElement('label');label.textContent=name;let field;if(options){field=document.createElement('select');for(const option of options){const entry=document.createElement('option');entry.value=option;entry.textContent=option||'（未设置）';field.append(entry);}field.value=value||'';}else{field=document.createElement('input');field.value=value||'';field.placeholder=placeholder||'';}field.onchange=()=>vscode.postMessage({type:'setAttribute',start:node.start,name,value:field.value});label.append(field);host.append(label);}
function properties(node){const host=byId('properties');host.innerHTML='';const title=document.createElement('h2');title.textContent='属性';host.append(title);if(!node){const empty=document.createElement('p');empty.textContent='在组件树或画布选择节点。';host.append(empty);return;}const name=document.createElement('p');name.textContent=node.displayName||node.tag;host.append(name);const attrs=node.attrs||{};const layout=['Width','Height','MinWidth','MinHeight','Margin','Padding','Gap','FlexGrow','Position','Anchor','Left','Top','Right','Bottom','Align','Justify'];const layoutTitle=document.createElement('h3');layoutTitle.textContent='布局';host.append(layoutTitle);const note=document.createElement('p');note.className='property-note';note.textContent='数值默认按 px；Margin/Padding 可填 8 或 8 12；锚点配合绝对定位使用。';host.append(note);const grid=document.createElement('div');grid.className='property-grid';for(const key of layout){const value=attrs[key]||'';const options=key==='Position'?['','flow','absolute']:key==='Anchor'?['','top-left','top','top-right','left','center','right','bottom-left','bottom','bottom-right']:key==='Align'?['','start','center','end','stretch']:key==='Justify'?['','start','center','end','between','around','evenly']:undefined;propertyInput(grid,node,key,value,key==='Margin'||key==='Padding'?'8':key==='Width'||key==='Height'?'auto':'',options);}host.append(grid);const otherTitle=document.createElement('h3');otherTitle.textContent='其他属性';host.append(otherTitle);for(const key of Object.keys(attrs)){if(!layout.includes(key))propertyInput(host,node,key,attrs[key],'');}const add=document.createElement('button');add.textContent='添加 Panel 子节点';add.onclick=()=>vscode.postMessage({type:'addChild',start:node.start,tag:'Panel'});host.append(add);}
function draw(){const outlineHost=byId('outline');outlineHost.innerHTML='';const canvas=byId('canvas');canvas.innerHTML='';if(!model?.root)return;const select=byId('preview');const states=previews(model.root);const retained=select.value;select.innerHTML='';for(const state of states){const option=document.createElement('option');option.value=state.start;option.textContent=state.displayName||state.attrs?.['x:Name']||'预览';select.append(option);}if([...select.options].some(option=>option.value===retained))select.value=retained;byId('preview-label').style.display=states.length?'':'none';outline(model.root,outlineHost);canvas.append(renderNode(model.root));properties(currentNode(model.root,selected));const diagnostics=byId('diagnostics');diagnostics.innerHTML='';for(const item of model.diagnostics||[]){const line=document.createElement('p');line.textContent='⚠ '+item.message;diagnostics.append(line);}applyHighlights();}
