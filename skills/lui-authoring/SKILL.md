...
7. 多个业务 Modal 可以共存时，由宿主的集中式弹窗管理器在真实打开或恢复后动态置顶，并保持视觉 `zIndex` 与原生 overlay 输入栈一致。不要在各页面分散填写固定层级；通知、教程和阻断操作继续使用 `MountGlobalOverlay`。

## 完成检查

验证语法、配对后端、公开属性、目录登记和示例使用到的宿主能力。交互至少覆盖真实数据、事件回调与可见刷新；列表覆盖空集合和原始项身份。嵌套 Modal 同时检查视觉层级、`UI.GetTopOverlay()` 和关闭后的父层恢复。只有静态预览证据时明确保留运行时验证项。
