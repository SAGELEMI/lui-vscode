...
export const ATTRIBUTE_DEFINITIONS: Record<string, AttributeDefinition> = {
  ...
  Variant: { kind: "enum", options: ["主要", "次要"] }, Disabled: { kind: "enum", options: ["是", "否"] }, CloseOnOverlay: { kind: "enum", options: ["是", "否"] }, ShowCloseButton: { kind: "enum", options: ["是", "否"] }, Edges: { kind: "enum", options: ["全部", "无", "水平", "垂直"] }, Mode: { kind: "enum", options: ["内边距", "外边距"] }, NativeMenuInset: { kind: "enum", options: ["是", "否"] }
};
