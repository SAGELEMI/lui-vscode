...
预算逐次检查底层文本测量入口，缓存已完成的测量步骤，下一帧继续未完成部分。离屏队列在 EndFrame 执行，先完成所有 Runtime 的可见 Update/Render 工作，再轮流推进各列表；队列最多登记 128 个列表，每次推进一行。尚未完成几何的控件暂停绘制子节点及命中，完成后恢复。关闭或卸载会取消队列登记并释放输入焦点。
