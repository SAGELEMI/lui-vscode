...
弹窗引用是实际 UI.Modal。LUI 不决定多个业务弹窗的打开顺序：层级/ZIndex 只影响树与绘制顺序，不会同步原生 overlay 输入栈。允许嵌套弹窗时，宿主应在集中式打开器中按真实打开/恢复顺序动态置顶；不要在每个页面分散填写固定层级。通知、教程和阻断操作使用 MountGlobalOverlay。
