...
| 详情弹窗出现在父弹窗下方，或视觉与点击对象不一致 | 同时检查两个 Modal 的兄弟 `zIndex` 与 `UI.GetTopOverlay()`。输入栈正确但视觉倒序时，由业务宿主的集中式弹窗管理器在真实打开/恢复后将新 Modal 提升到兄弟最大层级加一；不要逐页写固定数字，也不要修改 LUI Runtime 适配某个业务流程 |
