...
当前手册适用于 LUI Studio / UrhoX Lua Runtime **3.0.1**；旧的背景、颜色、边框颜色、边框宽度和圆角写法继续兼容。以当前实现为准，不套用标准 WPF 或浏览器 CSS 的全部能力。

3.0.1 的 Studio 将原始声明交给正式 Runtime，保留绑定、条件、重复项、组件和虚拟列表模板，共用布局与测量；场景、受控页面与复用控件分别登记。项目可使用 `NotifyChanged` 通知局部变化。单文件 Studio 固定使用结构示意和绑定内联的 `预览内容`。项目级“运行”在带跨源隔离响应头的独立浏览器中执行 Maker 项目入口，并支持设备尺寸切换、游戏操作及源码选取。
